...
def get_certs(certs,app):
    OScommand = ['mkdir ' + app,
                'gsutil cp '+ str(certs) + '* '  + app + '/',
                'chmod 600 ' + app + '/*']
...
class SQLReader(dataflow_io.NativeSourceReader):
    ...
    def _build_connection_mysql(self):
        ...
        if self.source.wrapper == MySQLWrapper:
            ...
            import os
            import logging
            logging.info("Proceso de Certificados en _build_connection_mysql: ")
            get_certs(self.source.certs, self.source.app)
            connection = pymysql.connect(host=self.source.host, port=int(self.source.port),
                                         ...
                                         ssl_ca=self.source.sslrootcert,
                                         ssl_cert=self.source.sslcert,
                                         ssl_key=self.source.sslkey,
            ...
            connection = psycopg2.connect(host=self.source.host, port=int(self.source.port),
                                          user=self.source.username, password=self.source.password,
                                          database=self.source.database, sslmode=self.source.sslmode,
                                          sslrootcert=self.source.sslrootcert, 
                                          sslcert=self.source.sslcert, 
                                          sslkey=self.source.sslkey
                                          )
...
class SQLSouceInput(object):
    def __init__(self, host=None, port=None, username=None, password=None,
                 ...
                 sslmode=None, sslrootcert=None, sslcert=None, sslkey=None, certs=None, app=None,
        ...
        self.runtime_params = ['host', 'port', 'username', 'password', 'database',
                               ...
                               'sslkey', 'sslcert', 'sslrootcert', 'sslmode',
                               'batch', 'schema_only', 'app']
...
class ReadFromSQL(beam.PTransform):
    ...
    def expand(self, pcoll):
        import logging
        logging.info('INICIA EJECUCION DE ReadFromSQL')
...
class PaginateQueryDoFn(beam.DoFn):
        ...
        def process(self, query, *args):
            import logging
            logging.info('INICIA EJECUCION DE PaginateQueryDoFn:Antes de SQLSource')
            source = SQLSource(*self.args, **self.kwargs)
            SQLSouceInput._build_value(source, source.runtime_params)
            logging.info('INICIA EJECUCION DE PaginateQueryDoFn:Despues de SQLSource')
            ...
            try:
                ...
                for offset in offsets:
                    logging.info('INICIA EJECUCION DE paginated_query - '+str(datetime.datetime.now()))
                    paginated_query, status = self.paginated_query(query, batch, offset)
                    queries.append(paginated_query)
                    logging.info(("paginated query", paginated_query))
                    if not status:
                        break
...
class SQLSourceDoFn(beam.DoFn):
    ...
    def _create_connection(self):
        if self.source.wrapper == MySQLWrapper:
            import pymysql
            import logging
            logging.info("Proceso de Certificados en _create_connection: ")
            get_certs(self.source.certs, self.source.app)
